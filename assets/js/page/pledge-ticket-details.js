...
function call() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#price").append(
        parseFloat(this.results.Price).toLocaleString("thai", {
      ...
      var redeemPrice =
        parseFloat(this.results.Price) +
        (parseFloat(this.results.InterestRate) *
          parseFloat(this.results.Price)) /
          100;
      $("#redeemPrice").append(
        redeemPrice.toLocaleString("thai", { minimumFractionDigits: 2 })
      );
...
function display(results) {
  ...
  if (typeof results.data[0] != "undefined") {
    ...
    results.data.forEach(function(data) {
      ...
      tableData = [];
      tableData.push(
        '<input type="hidden" class="o_PledgeTicketID" value="' +
          data.PledgeTicketID +
          '" />' +
          '<input type="hidden" class="o_PriceRate" value="' +
          data.PriceRate +
          '" />' +
          '<input type="hidden" class="o_ContinueDate" value="' +
          data.ContinueDate +
          '" />' +
          index++
      );
